...
Use order management to add a *Receipt* or *Category* to a transaction made with the eCom, Recurring, or ePayment API. The receipt is connected to a `orderId` (called `reference` in the ePayment API). The `orderId` is what **you** use when either initiating an eCom payment or creating a recurring charge.
...
1. Initiate an eCom, ePayment, or recurring payment
